...
function tokenizeQuery(q: string) {
  return q
    ...
    .filter(Boolean)
    .filter((x) => x.length >= 3)
    .filter((x) => !["the", "and", "for", "with", "from", "that", "this", "project", "projects"].includes(x));
}
...
export async function GET(req: NextRequest) {
  ...
  // 1) try broad natural-language matching
  ...
  // 2) fallback: token-based matching
  ...
  const inferred = inferProjectSignals(topProject, companiesRes.rows.length);
