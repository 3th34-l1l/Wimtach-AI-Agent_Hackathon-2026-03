...
function buildHtmlEmail(opts: {
  ...
  const { companyName, tagline, primary, to, subject, formName, stamp, formData, narrative, bodyText } = opts;
...
export async function POST(req: Request) {
  try {
    ...
    if (!to || !subject || (!body && !payload?.formData && !payload?.narrative)) {
      return ok({ ok: false, error: "Missing email fields (need to/subject and body or structured fields)" });
    }
    ...
    if (!host || !user || !pass) {
      return ok({ ok: false, error: "SMTP env missing. Need SMTP_HOST/SMTP_USER/SMTP_PASS." });
    }
    ...
    const formData =
      payload?.formData && typeof payload.formData === "object" ? (payload.formData as Record<string, any>) : undefined;
    ...
    return ok({ ok: true, message: "Email sent" });
  } catch (err: any) {
